...
#define _W25QXX_STARTUP_INFO        0
#define FLASH_USE_MUTEX             1

#if (_W25QXX_STARTUP_INFO == 1)
#include <stdio.h>
#endif
...
#define W25qxx_Delay(delay)     UserDelayUs(delay)
#endif
...
SPI_HandleTypeDef *g_flashSpi = NULL;

//###################################################################################################################
uint8_t W25qxx_Spi(uint8_t  Data)
{
    uint8_t ret;
    HAL_SPI_TransmitReceive(g_flashSpi, &Data, &ret, 1, 100);
    return ret;
}
//###################################################################################################################
...
#if (_W25QXX_STARTUP_INFO==1)
    printf("w25qxx Page Size: %u Bytes\r\n", w25qxx.PageSize);
...
bool W25qxx_IsEmptyPage(uint32_t Page_Address, uint32_t OffsetInByte, uint32_t NumByteToCheck_up_to_PageSize)
{
    ...
    for (i = OffsetInByte; i < w25qxx.PageSize; i += sizeof(pBuffer)) {
        ...
        W25qxx_Spi(0);
        HAL_SPI_Receive(g_flashSpi, pBuffer, sizeof(pBuffer), 100);
        HAL_GPIO_WritePin(_W25QXX_CS_GPIO, _W25QXX_CS_PIN, GPIO_PIN_SET);
    ...
    if ((w25qxx.PageSize + OffsetInByte) % sizeof(pBuffer) != 0) {
        ...
        for (; i < w25qxx.PageSize; i++) {
            ...
            W25qxx_Spi(WorkAddress & 0xFF);
            W25qxx_Spi(0);
            HAL_SPI_Receive(g_flashSpi, pBuffer, 1, 100);
            HAL_GPIO_WritePin(_W25QXX_CS_GPIO, _W25QXX_CS_PIN, GPIO_PIN_SET);
            if (pBuffer[0] != 0xFF)
                goto NOT_EMPTY;
        }
...
{
    uint32_t writeBytes, writeCount = 0;
    if (addr % w25qxx.PageSize + size > w25qxx.PageSize) {
        //across page
        writeBytes = w25qxx.PageSize - addr % w25qxx.PageSize;
        W25qxx_WritePageBytes(pBuffer, addr, writeBytes);
        writeCount += writeBytes;
    }
    while (writeCount < size) {
        writeBytes = ((size - writeCount) > w25qxx.PageSize) ? w25qxx.PageSize : (size - writeCount);
        W25qxx_WritePageBytes(pBuffer + writeCount, addr + writeCount, writeBytes);
        writeCount += writeBytes;
    }
}


void W25qxx_WritePageBytes(uint8_t *pBuffer, uint32_t addr, uint32_t num)
{
#if (FLASH_USE_MUTEX)
    osMutexAcquire(g_flashMutex, osWaitForever);
#endif
    ...
    //    NumByteToWrite_up_to_PageSize = w25qxx.PageSize-OffsetInByte;
#if (_W25QXX_DEBUG==1)
    printf("w25qxx WriteBytes addr %lu, Writes %lu Bytes, begin...\r\n", addr, num);
    //W25qxx_Delay(100);
    uint32_t StartTime = HAL_GetTick();
#endif
    W25qxx_WaitForWriteEnd();
    ...
    W25qxx_Spi((addr & 0xFF00) >> 8);
    W25qxx_Spi(addr & 0xFF);
    HAL_SPI_Transmit(g_flashSpi, pBuffer, num, 100);
    HAL_GPIO_WritePin(_W25QXX_CS_GPIO, _W25QXX_CS_PIN, GPIO_PIN_SET);
    W25qxx_WaitForWriteEnd();
#if (_W25QXX_DEBUG==1)
    ...
    printf("\r\n");
    printf("w25qxx WriteBytes done after %lu ms\r\n", StartTime);
    W25qxx_Delay(100);
#endif
    W25qxx_Delay(1);
    w25qxx.Lock = 0;
...
void W25qxx_WritePage(uint8_t *pBuffer, uint32_t Page_Address, uint32_t OffsetInByte, uint32_t NumByteToWrite_up_to_PageSize)
{
    ...
    HAL_SPI_Transmit(g_flashSpi, pBuffer, NumByteToWrite_up_to_PageSize, 100);
    HAL_GPIO_WritePin(_W25QXX_CS_GPIO, _W25QXX_CS_PIN, GPIO_PIN_SET);
    W25qxx_WaitForWriteEnd();
#if (_W25QXX_DEBUG==1)
    StartTime = HAL_GetTick() - StartTime;
...
void W25qxx_ReadBytes(uint8_t* pBuffer, uint32_t ReadAddr, uint32_t NumByteToRead)
{
    ...
    W25qxx_Spi(0);
    HAL_SPI_Receive(g_flashSpi, pBuffer, NumByteToRead, 2000);
    HAL_GPIO_WritePin(_W25QXX_CS_GPIO, _W25QXX_CS_PIN, GPIO_PIN_SET);
#if (_W25QXX_DEBUG==1)
    StartTime = HAL_GetTick() - StartTime;
...
void W25qxx_ReadPage(uint8_t *pBuffer, uint32_t Page_Address, uint32_t OffsetInByte, uint32_t NumByteToRead_up_to_PageSize)
{
    ...
    W25qxx_Spi(Page_Address & 0xFF);
    W25qxx_Spi(0);
    HAL_SPI_Receive(g_flashSpi, pBuffer, NumByteToRead_up_to_PageSize, 100);
    HAL_GPIO_WritePin(_W25QXX_CS_GPIO, _W25QXX_CS_PIN, GPIO_PIN_SET);
#if (_W25QXX_DEBUG==1)
    StartTime = HAL_GetTick() - StartTime;
