...
static bool I2CIO_WaitAck(const I2CIO_Cfg_t *cfg)
{
    ...
    while (HAL_GPIO_ReadPin(cfg->SDA_PORT, cfg->SDA_PIN)) {
        tick--;
        if (!tick) {
            return false;
        }
...
static uint8_t I2CIO_ReadByte(const I2CIO_Cfg_t *cfg)
{
    ...
    for (i = 0; i < 8; i++) {
        ...
        HAL_GPIO_WritePin(cfg->SCL_PORT, cfg->SCL_PIN, GPIO_PIN_SET);
        I2CIO_Delay(100);
        if (HAL_GPIO_ReadPin(cfg->SDA_PORT, cfg->SDA_PIN)) {
            readValue |= 0x01;
        }
    }
    HAL_GPIO_WritePin(cfg->SCL_PORT, cfg->SCL_PIN, GPIO_PIN_RESET);
...
void I2CIO_Init(I2CIO_Cfg_t *cfg, GPIO_TypeDef *SCL_Port, uint16_t SCL_Pin, GPIO_TypeDef *SDA_Port, uint16_t SDA_Pin)
{
    ...
    gpioInit.Pin = cfg->SCL_PIN;
    gpioInit.Pull = GPIO_PULLUP;
    gpioInit.Speed = GPIO_SPEED_FREQ_LOW;
    HAL_GPIO_Init(cfg->SCL_PORT, &gpioInit);

    gpioInit.Mode = GPIO_MODE_OUTPUT_OD;
    gpioInit.Pin = cfg->SDA_PIN;
    gpioInit.Pull = GPIO_PULLUP;
    gpioInit.Speed = GPIO_SPEED_FREQ_LOW;
    HAL_GPIO_Init(cfg->SDA_PORT, &gpioInit);
