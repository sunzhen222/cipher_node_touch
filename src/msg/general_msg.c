...
uint32_t PubBufferMsg(uint32_t messageID, const void *buffer, uint32_t length)
{
    Message_t msg;
    ...
    for (uint32_t i = 0;; i++) {
        ...
        if (queue == NULL) {
            if (i == 0) {
                printf("msg id err,msg id=0x%08lX\r\n", messageID);
        ...
        if (err != osOK) {
            SramFree(msg.buffer);
            printf("msg bugger write queue err,queue=0x%p,msgID=0x%08lX,errid=0x%08X\r\n", queue, messageID, err);
            continue;
        }
    }
}

uint32_t PubValueBufferMsg(uint32_t messageID, uint32_t value, const void *buffer, uint32_t length)
{
    Message_t msg;
    osMessageQueueId_t queue;
    osStatus_t err = osOK;
...
uint32_t PubValueMsg(uint32_t messageID, uint32_t value)
{
    ...
    for (uint32_t i = 0;; i++) {
        ...
        err = osMessageQueuePut(queue, &msg, 0, 0);
        if (err != osOK) {
            printf("msg write queue err,queue=0x%p,msgID=0x%08lX,errid=0x%08X\r\n", queue, messageID, err);
            continue;
        }
    }
}
