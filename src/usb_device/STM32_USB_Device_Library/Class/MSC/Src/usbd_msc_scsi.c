...
int8_t SCSI_ProcessCmd(USBD_HandleTypeDef *pdev, uint8_t lun, uint8_t *cmd)
{
    ...
    //printf("%X\n", params[0]);
    if (cmd[0] == SCSI_TEST_UNIT_READY && lastParam[0] == SCSI_TEST_UNIT_READY && lastParam[1] == SCSI_WRITE10) {
        printf("file copy over\n");
        FsMount();
        FIL fp;
        uint32_t fileSize;
        FRESULT res = f_open(&fp, "0:update.bin", FA_OPEN_EXISTING | FA_READ);
        if (res) {
            printf("open error\n");
        } else {
            fileSize = f_size(&fp);
        ...
        SendUiMsg(UI_MSG_CODE_FILE_SYSTEM_REFRESH, NULL, 0);
    }
