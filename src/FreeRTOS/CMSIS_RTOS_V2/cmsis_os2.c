...
osStatus_t osTimerReset(osTimerId_t timer_id)
{
    TimerHandle_t hTimer = (TimerHandle_t)timer_id;
    osStatus_t stat = osErrorResource;
    BaseType_t yield;
    ...
    if (hTimer == NULL) {
        ...
        if (IS_IRQ()) {
            yield = pdFALSE;
            if (xTimerResetFromISR(hTimer, &yield) != pdPASS) {
                portYIELD_FROM_ISR(yield);
            }
        } else {
...
static StackType_t  Timer_Stack[configTIMER_TASK_STACK_DEPTH] __attribute__((section(".ccmram")));
