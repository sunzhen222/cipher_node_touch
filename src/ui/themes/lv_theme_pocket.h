
#ifndef LV_THEME_POCKET_H
#define LV_THEME_POCKET_H

#ifdef __cplusplus
extern "C" {
#endif


/*********************
 *      INCLUDES
 *********************/
#include "src/themes/lv_theme.h"
#include "src/display/lv_display.h"

#if LV_USE_THEME_POCKET

/*********************
 *      DEFINES
 *********************/

/**********************
 *      TYPEDEFS
 **********************/

/**********************
 * GLOBAL PROTOTYPES
 **********************/

/**
 * Initialize the theme
 * @param disp pointer to display
 * @return a pointer to reference this theme later
 */
lv_theme_t * lv_theme_pocket_init(lv_display_t * disp);

/**
* Check if the theme is initialized
* @return true if default theme is initialized, false otherwise
*/
bool lv_theme_pocket_is_inited(void);

/**
 * Get pocket theme
 * @return a pointer to pocket theme, or NULL if this is not initialized
 */
lv_theme_t * lv_theme_pocket_get(void);

/**
 * Deinitialize the pocket theme
 */
void lv_theme_pocket_deinit(void);

/**********************
 *      MACROS
 **********************/

#endif

#ifdef __cplusplus
} /*extern "C"*/
#endif

#endif /*LV_THEME_POCKET_H*/
